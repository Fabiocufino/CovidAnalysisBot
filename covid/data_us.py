...
def process_covid_data_us(data: pd.DataFrame, run_date: pd.Timestamp):
    ...
    #return data.loc[idx[:, :(run_date - pd.DateOffset(1))], ["positive", "total", "all_cases"]]
    return data


def get_and_process_covid_data_us(run_date: pd.Timestamp):
    """ Helper function for getting and processing COVIDTracking data at once """
    data = get_raw_covid_data_us()
    data = process_covid_data_us(data, run_date)
    return data
